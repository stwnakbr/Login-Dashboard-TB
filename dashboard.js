...
function updateProjectDetailsByActivities(filteredActivities) {
  const detailsList = document.getElementById("projectDetailsList");
  ...
  if (projects.length === 0) {
    detailsList.innerHTML = `<div class="no-data">Tidak ada proyek</div>`;
    return;
  }
  ...
  detailsList.innerHTML = projects
    .map(
      (project) => `
    <div class="detail-item" onclick="showProjectActivitiesModal('${project.id}')">
      <div class="detail-item-header">
        <div class="detail-item-icon">
          <i class="fas fa-folder"></i>
        </div>
        <div class="detail-item-info">
          ...
          <span class="detail-item-count">
            ${escapeHtml(project.prokerBacklog)} • ${escapeHtml(project.departemen)}
          </span>
        </div>
      </div>
      ...
      <div class="detail-item-meta">
        <span class="detail-meta-badge">
          <i class="fas fa-tasks"></i> ${project.activities.length} activities
        </span>
      </div>
      ...
      <div class="detail-item-progress">
        <div class="progress-bar-wrapper">
          <div class="progress-bar-fill"
            style="width:${project.activities.length
              ? (project.completed / project.activities.length) * 100
              : 0}%">
          </div>
        </div>
        <span class="progress-percentage">
          ${project.activities.length
            ? ((project.completed / project.activities.length) * 100).toFixed(0)
            : 0}%
        </span>
      </div>
    </div>
...
function showDepartmentDetail(departmentName) {
  const headerContainer = document.getElementById("departmentDetailHeader");
  const projectList = document.getElementById("projectDetailsList");

  if (!headerContainer || !projectList) return;

  let activities = [];

  if (departmentName === "All") {
    activities = projectsData.filter(task => !task.isTotalRow);
  } else {
    activities = (departmentData[departmentName] || []).filter(
      task => !task.isTotalRow
    );
  }
    ...
    headerContainer.innerHTML = `
      <div class="department-detail-header">
        <h3>
          ...
          ${departmentName === "All" ? "All Departments" : escapeHtml(departmentName)}
        </h3>
      </div>
    `;
  ...
  activities.forEach(task => {
    ...
    projectGroups[id].total++;

    if (task.status === "Complete") {
      projectGroups[id].completed++;
    } else if (task.status === "In Progress") {
      projectGroups[id].inProgress++;
    } else {
      projectGroups[id].outstanding++;
    }
  });
  ...
  projects.forEach(project => {
    if (project.completed === project.total) {
      completeCount++;
    } else if (project.inProgress > 0 || project.completed > 0) {
      inProgressCount++;
    } else {
      outstandingCount++;
    }
  ...
  headerContainer.innerHTML = `
    <div class="department-detail-header">
      ...
      <div class="status-summary">
        <div class="detail-stat">
          <span class="detail-stat-label">Complete</span>
          <span class="detail-stat-value">${completeCount}</span>
        </div>
        <div class="detail-stat">
          <span class="detail-stat-label">In Progress</span>
          <span class="detail-stat-value">${inProgressCount}</span>
        </div>
        <div class="detail-stat">
          <span class="detail-stat-label">Outstanding</span>
          <span class="detail-stat-value">${outstandingCount}</span>
        </div>
      </div>
    </div>
  `;
  ...
  /* ===============================
     RENDER PROJECT LIST (FILTERED)
     =============================== */

  updateProjectDetailsByActivities(activities);
...
function showDetailModal() {
  ...
  modal.classList.add("show"); // 🔥 INI KUNCINYA
  document.body.style.overflow = "hidden";
  populateDetailModal();
}
...
function backToProjectsList() {
  ...
  const filters = modal.querySelector(".detail-filters");

  title.innerHTML = `<i class="fas fa-list-alt"></i> All Projects`;
